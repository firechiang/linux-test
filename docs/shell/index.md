...
#### 三、数组简单使用（注意：数组元素从0开始，还有元素被删除后，该元素的下标依然被占用，只是没有值了，所以元素被删除后，其它元素的下标不会改变）
```bash
$ aaa=(2 2 3 4 5)                         # 定义数组变量
...
$ echo ${aaa[@]:1:3}                      # 打印数组元素从第1个开始，取3个
$ echo ${aaa[@]:1}                        # 打印数组元素从第1个开始，到最后一个

$ echo ${aaa[@]/2/22}                     # 将数组元素里面包含2的替换成22（只替换第一个包含2的元素），最后打印返回替换后数组所有元素 
$ echo ${aaa[@]//2/11}                    # 将数组元素里面包含2的替换成22（只替全部包含2的元素），最后打印返回替换后数组所有元素

$ unset aaa[1]                            # 删除数组里面下标为2的元素（注意：下标为2的元素被删除后，下标2里面就没有元素，但下标依然被占用）
$ unset aaa                               # 清空数组里面所有的元素

$ for a in ${aaa[@]};do echo $a;done      # 遍历数组aaa里面所有元素
```
...
#### 十三、while read循环读取文件每一行数据简单使用（注意：while read 循环读取命令默认会用空格或tab键分割数据（我们只需定义变量将分割后的数据接住即可，[详情可参考](https://github.com/firechiang/linux-test/blob/master/docs/shell-mysql-operation.md#%E4%BA%94%E5%B0%86%E7%89%B9%E5%AE%9A%E6%96%87%E6%9C%AC%E6%95%B0%E6%8D%AE%E5%AF%BC%E5%85%A5%E5%88%B0mysql%E6%B3%A8%E6%84%8Fmysql%E6%9C%AC%E8%BA%AB%E6%9C%89%E8%87%AA%E5%B8%A6%E7%9A%84load-data%E5%91%BD%E4%BB%A4%E5%8F%AA%E8%83%BD%E5%AF%BC%E5%85%A5%E5%9B%BA%E5%AE%9A%E6%A0%BC%E5%BC%8F%E7%9A%84%E6%95%B0%E6%8D%AE)）
```bash
#!/bin/bash
#
